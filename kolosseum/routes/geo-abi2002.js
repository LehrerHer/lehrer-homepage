...
const FELDER = ['vorname', 'nachname', 'maedchenname', 'strasse', 'plz', 'wohnort', 'telefonnummer', 'social', 'email'];
...
router.put('/:id', checkSecret, limiter, (req, res) => {
  ...
  try {
    db.prepare(
      `UPDATE geo_abi2002
       SET vorname = ?, nachname = ?, maedchenname = ?, strasse = ?, plz = ?, wohnort = ?, telefonnummer = ?, social = ?, email = ?, updated_at = CURRENT_TIMESTAMP
       WHERE id = ?`
    ).run(werte.vorname, werte.nachname, werte.maedchenname, werte.strasse, werte.plz, werte.wohnort, werte.telefonnummer, werte.social, werte.email, id);
    res.json({ ok: true });
  } catch (e) {
